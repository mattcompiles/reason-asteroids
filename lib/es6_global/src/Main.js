...
function update(state) {
  ...
  var match$1 = state[/* framesBetweenWave */5];
  var newState;
  if (match !== 0) {
    ...
    newState = /* record */[
      ...
      /* performanceStats */state[/* performanceStats */1],
      /* screenSize */state[/* screenSize */2],
      /* asteroids */Asteroid.makeMany(/* Large */0, state[/* screenSize */2], state[/* wave */4] + 3 | 0, /* [] */0),
      /* wave */state[/* wave */4] + 1 | 0,
      /* framesBetweenWave */0
    ];
  } else {
    var newrecord = updateWave(state).slice();
    newrecord[/* framesBetweenWave */5] = match$1 + 1 | 0;
    newState = newrecord;
  }
...
function draw(ctx, state) {
  ...
  return Draw_canvas.wave(ctx, state[/* wave */4], screenSize);
}
