...
function checkBulletAsteroidCollisions(asteroids, bullets) {
  var match = List.partition((function (asteroid) {
          return List.exists((function (bullet) {
  ...
  return List.append(match[1], List.flatten(List.map(Asteroid.destroy, match[0])));
}
...
function update(state) {
  ...
  var asteroids = List.map((function (param) {
        ...
        }), checkBulletAsteroidCollisions(state[/* asteroids */3], ship[/* bullets */6]));
  return /* record */[
          /* ship */ship,
