...
function calcVelocity(state, param) {
  var up = param[/* up */2];
  var newrecord = state.slice();
  newrecord[/* shipVelocity */1] = normalizeVelocity(up !== 0 ? Vec.add(state[/* shipVelocity */1], state[/* shipThrust */2]) : Vec.multiply(state[/* shipVelocity */1], 0.94));
  return newrecord;
}

function calcPosition(state) {
  var newrecord = state.slice();
  newrecord[/* shipPosition */0] = Utils.normalizePosition(state[/* screenSize */7], Vec.add(state[/* shipPosition */0], state[/* shipVelocity */1]));
  return newrecord;
}
...
function shoot(state) {
  return /* :: */[
          Bullet.make(state[/* shipPosition */0], state[/* shipAngle */3]),
          state[/* bullets */9]
        ];
}
