...
function updateBullets(ship, bullets) {
  var newrecord = ship.slice();
  newrecord[/* bullets */6] = List.map(Bullet.update, bullets);
  return newrecord;
}

function update(ship, bullets, screenSize) {
  return calcWeaponState(removeOldBullets(updateBullets(calcPosition(calcVelocity(calcThrust(calcAngle(ship, Controls.activeInput), Controls.activeInput), Controls.activeInput), screenSize), bullets), screenSize), Controls.activeInput);
}

function destroy(ship) {
  var newrecord = ship.slice();
  newrecord[/* destroyed */8] = /* true */1;
  return newrecord;
}
