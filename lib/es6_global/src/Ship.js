...
function normalizePosition(param, position) {
  var screenSize = param[/* screenSize */7];
...
function calcPosition(state) {
  var newrecord = state.slice();
  newrecord[/* shipPosition */0] = normalizePosition(state, Vec.add(state[/* shipPosition */0], state[/* shipVelocity */1]));
  return newrecord;
}
